...
export default function PaymentsScreen({ navigation }: Props) {
  ...
  const total = payments.reduce((sum, t) => sum + (Number(t.amount) || 0), 0);
            ...
            return (
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  {date ? <Text style={styles.rowMeta}>{date}</Text> : null}
                </View>
                <Text style={[styles.rowAmount, styles.expenseText]}>
                  -£{amt.toFixed(2)}
                </Text>
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  summaryRow: { flexDirection: 'row', marginBottom: 12, alignItems: 'stretch' },
  summaryCard: {
    flex: 1,
    padding: 12,
    borderRadius: 12,
